...
def test_1_userDeslogado_mobile(driver, setup_site, wait):
    ...
    mobile_click_strict(driver, MOBILE_FILTER_CONSERVACAO_OPEN, wait=wait, retries=4, sleep_between=0.25)
    visible(driver, MOBILE_FILTER_CONSERVACAO_CONGELADO, wait=wait)
    mobile_click_strict(driver, MOBILE_FILTER_CONSERVACAO_CONGELADO, wait=wait, retries=4, sleep_between=0.25)
    ...
    time.sleep(2)
    safe_click_loc_retry(driver, MOBILE_MENU_PARENT_NEXT("bovinos-premium"), wait=wait, retries=4, sleep_between=0.25)
    time.sleep(1)
    ...
    safe_click_loc_retry(driver, MOBILE_MENU_HAMBURGER, wait=wait, retries=4, sleep_between=0.25)
    time.sleep(2)
    safe_click_loc_retry(driver, MOBILE_MENU_PARENT_NEXT("pescados"), wait=wait, retries=4, sleep_between=0.25)
    time.sleep(1)
    safe_click_loc_retry(driver, MOBILE_MENU_SEE_ALL, wait=wait, retries=4, sleep_between=0.25)
    wait_category_loaded(wait, driver)
    wait.until(EC.visibility_of_element_located(SORTER_SELECT))
